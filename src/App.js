...
// =========================================
  fetchShoppingList = () => {
    fetch('https://shopping-back.herokuapp.com/list')
        .then(data => data.json())
        .then(jData => {
          this.setItems(jData)
        })
...
// =========================================
  handleListUpdate = (item, arrayIndex, currentArray)=>{
    ...
    fetch(`https://shopping-back.herokuapp.com/list/${item.id}`, {
      body: JSON.stringify(item),
      method: 'PUT',
      headers: {
        'Accept': 'application/json, text/plain, */*',
  ...
  handleListDelete =(itemId, arrayIndex, currentArray)=>{
    fetch(`https://shopping-back.herokuapp.com/list/${itemId}`, {
      method: 'DELETE'
    })
    .then(data => {
      this.fetchShoppingList()
...
fetchRefrigerator = () => {
  ...
  fetch('https://shopping-back.herokuapp.com/refrigerator')
      .then(data => data.json())
      .then(jData => {
...
// =========================================
  handleFridgeUpdate = (item, arrayIndex, currentArray)=>{
    ...
    fetch(`https://shopping-back.herokuapp.com/refrigerator/${item.id}`,{
      body: JSON.stringify(item),
      method: 'PUT',
      headers: {
        'Accept': 'application/json, text/plain, */*',
  ...
  handleFridgeDelete =(itemId, arrayIndex, currentArray)=>{
    fetch(`https://shopping-back.herokuapp.com/refrigerator/${itemId}`, {
      method: 'DELETE'
    })
    .then(data => {
      this.fetchRefrigerator()
